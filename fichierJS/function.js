  ...
  for (let k=0; k < allRecipes.length; k++){
    ...
    for(let j=0; j<tabFiltre.length; j++){
      ...
      for (let l=0; l < allRecipes[k].ingredients.length; l++){
        if(allRecipes[k].ingredients[l].ingredient.toLowerCase().search(tabFiltre[j].toLowerCase())!= -1){
          counteurOk++
          //spanFilterSelectedClick.setAttribute("class", "ingredientFilterClick");
        }
      }
      //Evenement click appliance
      if(allRecipes[k].appliance.toLowerCase().search(tabFiltre[j].toLowerCase())!= -1){
        counteurOk++
      ...
      for (let m=0; m < allRecipes[k].ustensils.length; m++){
        if(allRecipes[k].ustensils[m].toLowerCase().search(tabFiltre[j].toLowerCase())!= -1){
          counteurOk++
